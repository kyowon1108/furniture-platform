...
- **자유 건설 모드**: 타일을 클릭하여 L자형, ㄷ자형 등 자유로운 형태의 방을 직접 설계할 수 있습니다.
...
- **키보드 단축키**: F(바닥), E(지우개), V(벽) 키로 빠르게 모드를 전환할 수 있습니다.
...
- **자유 형태 방 지원**: L자형 등 복잡한 형태의 방에서도 타일 기반 충돌 감지로 가구가 바닥 영역 안에만 배치됩니다.
- **부드러운 경계 슬라이딩**: 벽 경계에서 가구가 자연스럽게 미끄러지듯 이동합니다.
...
- **최적화된 네트워크**: 이벤트 쓰로틀링으로 서버 부하를 줄이면서도 부드러운 동기화를 유지합니다.
